...
def init_admin():
    admin = Admin("admin@123", generate_password_hash("admin", method="sha256"))
...
def index():
    return render_template(
        "home.html",
        students=db.session.query(func.count(Student.roll)).scalar(),
        warden="Partha Haldar",
        no_of_rooms=20,
        hostel_fees=1000,
        mess_fees=2000,
    )
...
def admin():
    if "admin" in session:
        ...
        return render_template(
            "dashboard.html",
            students=db.session.query(func.count(Student.roll)).scalar(),
            students_1st=db.session.query(Student)
            .filter(Student.year == "1st")
            .count(),
            students_2nd=db.session.query(Student)
            .filter(Student.year == "2nd")
            .count(),
            students_3rd=db.session.query(Student)
            .filter(Student.year == "3rd")
            .count(),
            students_4th=db.session.query(Student)
            .filter(Student.year == "4th")
            .count(),
            students_CSE=db.session.query(Student)
            .filter(Student.dept == "CSE")
            .count(),
            students_IT=db.session.query(Student).filter(Student.dept == "IT").count(),
            students_CT=db.session.query(Student).filter(Student.dept == "CT").count(),
        )
...
def show_applicants():
    if "admin" in session:
        students = (
            db.session.query(Applicants).filter(Applicants.status == "waiting").all()
        )
...
def search_student():
    if "admin" in session:
        if request.method == "POST":
            ...
            if name:
                student = (
                    db.session.query(Student).filter(Student.name == name.upper()).all()
                )
                if len(student) == 0:
    ...
    if "email" in session:
        ...
        if request.method == "POST":
            if (
                ...
                or not request.form["roll"]
                or not request.form["year"]
                or not request.form["dept"]
...
def pay_fees():
    if "email" in session:
        ...
        client = razorpay.Client(
            auth=("rzp_test_A4VvhETPM5RfxC", "MYpcmIHLh4o7kc9CZb6rGYDp")
        )
        payment = client.order.create(
            {"amount": int(300000), "currency": "INR", "payment_capture": "1"}
        )
...
def check_stat(email):
    if "email" in session and email == session["email"]:
        ...
        if student.status == "waiting":
            ...
            flash(
                "Your application has been approved. Contact the superintendent for further details.",
                "success",
            )
        else:
